...
# Security Group for Databases (RDS)
...
# Security Group for Cassandra DB
resource "aws_security_group" "cassandra_sg" {
  name        = "uce-trade-cassandra-sg"
  description = "Allow Cassandra traffic from MS5"
  vpc_id      = aws_vpc.main_vpc.id
...
# EC2 Instance for Cassandra (MS5)
resource "aws_instance" "cassandra_db" {
  ...
  user_data = base64encode(<<-EOF
    ...
    sudo docker run -d --name cassandra-db --restart always -p 9042:9042 -e CASSANDRA_CLUSTER_NAME=UCECluster cassandra:latest
  EOF
  )

  tags = {
    Name = "MS5 Cassandra Database"
  }
}
...
output "cassandra_endpoint" {
  value       = aws_instance.cassandra_db.private_ip
  description = "Cassandra connection endpoint (Private IP)"
}